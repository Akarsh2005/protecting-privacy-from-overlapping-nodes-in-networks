...
def betweenness_hiding(G, target_node, target_community, T=3):
    G_copy = G.copy()
    for _ in range(T):
        ...
        if candidates:
            G_copy.add_edge(target_node, max(candidates, key=lambda x: x[1])[0])
    return G_copy
...
if __name__ == "__main__":
    ...
    G_hidden = betweenness_hiding(G, target_node, target_community)
    result_hidden = detect_communities(G_hidden)
    node_communities_hidden = get_node_communities(result_hidden)
    visualize(G_hidden, node_communities_hidden, target_node, "Betweenness Hiding")
